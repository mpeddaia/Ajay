...
cxCal.prototype.show = function() {
	// close all existing open calendars
	...
	// object. if it exists and is visible - call its close() method to remove it
	var dl = calDivs.length;
	for (var i=0; i<dl; i++) {
		var objName = calDivs[i].id.substring(0, (calDivs[i].id.length - 6));
		if ((typeof(window[objName]) != "undefined") && (calDivs[i].style.visibility == "visible")) {
			window[objName].close();
		}
	}
...
cxCal.prototype.resetPosition = function() {
	var hGap = 7; // hardcoded x offset.
	var vGap = 0; // hardcoded y offset.
	var hIconSize = 20; // hardcoded icon size.
	var vIconSize = 20; // hardcoded icon size.
	var hCalSize = 162;
	var vCalSize = 167;

	// Regular offset of calendar...
	var x1 = getPositionLeft(this.inputID) + MM_findObj(this.inputID).clientWidth + hGap + hIconSize;
	var y1 = getPositionTop(this.inputID) + vGap + vIconSize;

	// if thats too far to the right or bottom, switch calendar to left/above the icon
	var x2 = getStageW(this.inputID);
	var y2 = getStageH(this.inputID);

	if ((x1+hCalSize) > x2) {
		x1 = getPositionLeft(this.inputID) + MM_findObj(this.inputID).clientWidth + hGap + hIconSize - hCalSize;
	}

	if ((y1+vCalSize) > y2) {
		y1 = getPositionTop(this.inputID) + vGap - 2 - vCalSize;
	}
...
cxCal.prototype.writeCalendar = function() {
	...
	var calString = '<html>';
	calString += '<head>';
	calString += '<link href="../images/components/calendar/' + this.skinID + '/calendar_css.css" rel="stylesheet" type="text/css">'
	calString += '</head>';
	...
	calString += '			<a href="javascript:parent.' + this.instanceName + '.close();"><img src="../images/components/calendar/' + this.skinID + '/cal_close_normal_btn.gif" width="10" height="11" border="0"></a>';
	calString += '		</div>';
				...
				if ((displayDate > 0) && (displayDate <= daysInMonth[this.displayMonth])) {
					...
					if (
						...
						if ((this.displayYear == this.selectedDate.getFullYear()) && (this.displayMonth == this.selectedDate.getMonth()) && (displayDate == this.selectedDate.getDate())) {
							...
							calString += '<td class="selected"><a href="javascript:parent.' + this.instanceName + '.setSelectedDate('+displayDate+');">'+displayDate+'</a></td>';
						} else {
	...
	if (this.displayYear > sDate.getFullYear()) {
		calString += '<a href="javascript:parent.' + this.instanceName + '.prevYear();"><img src="../images/components/calendar/' + this.skinID + '/cal_prev_year_normal.gif" width="12" height="12" border="0"></a>';
	} else {
	...
	if ((this.displayYear == sDate.getFullYear()) && (this.displayMonth <= sDate.getMonth())) {
		...
		calString += '<a href="javascript:parent.' + this.instanceName + '.prevMonth();"><img src="../images/components/calendar/' + this.skinID + '/cal_prev_month_normal.gif" width="12" height="12" border="0"></a>';
	}
	...
	if ((this.displayYear == eDate.getFullYear()) && (this.displayMonth >= eDate.getMonth())) {
		...
		calString += '<a href="javascript:parent.' + this.instanceName + '.nextMonth();"><img src="../images/components/calendar/' + this.skinID + '/cal_next_month_normal.gif" width="12" height="12" border="0"></a>';
	}

	// if next year is a valid year to display, display the next year link
	if (this.displayYear < eDate.getFullYear()) {
		calString += '<a href="javascript:parent.' + this.instanceName + '.nextYear();"><img src="../images/components/calendar/' + this.skinID + '/cal_next_year_normal.gif" width="12" height="12" border="0"></a>';
	} else {
...
/**
 * Redraws the calendar and sets layer z-index to top. The show method
 * hides any currently open calendars.
 */
...
/////// Global Functions Added to hide calendars on mouse click outside the current calendar///////


/**
 * Global variable to track calendar state. When it is at 0 it is safe to close any open calendar.
 * Functions triggered by the calendar object (next month/year, previous month/year) should set this
 * to 1 before exiting to ensure that the calendar stays open after the mouseclick.
 */
...
/**
 * Global event handler registration for mouse click on the window object.
 */
window.onclick=closeAllCals;
window.document.onclick=closeAllCals;


/**
 * Event handler for mouse click event. Sets a time out to the hide function for all calendars
 */
function closeAllCals() {
	calHideTimer = setTimeout("hideCalendars()",200);
}


/**
 * Hide function for all calendars. Closes the open calendar when the user clicks
 * outside the calendar area.
 */
function hideCalendars() {
	if (calStatus == 0) {
		// close all existing calendars
		var allDivs = document.getElementsByTagName('div');
		var calDivs = new Array(); // array of all divs that match the calendar div naming pattern
		var calDivRegExp = /\S+calDiv$/; // regular expression to match calendar div name against
...
// More support functions, these calculate the scroll offset on the screen

/**
 * finding total width of the document including scroll area
 */
function getStageW() {
    this.width = 0;
...
/**
 * finding total height of the document including scroll area
 */  
function getStageH() {
    this.height = 0;
