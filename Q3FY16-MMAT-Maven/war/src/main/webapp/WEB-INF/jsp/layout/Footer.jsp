...
<%
	...
	String combinedImagesLocation3 = request.getScheme() + "://"
			...
			// Q#FY13: sudaupad- TD 93640 - Use the cookie to identify if disti signin was already done:  - START
			String distiSignIn="NO";
	        ...
	        if (cookies != null) {
		      for (int i = 0; i < cookies.length; i++) {
		        if (cookies[i].getName().equals("sessionid")) {
		          distiSignIn = cookies[i].getValue();
		          break;
		        }
		      }
		    }
			...
			String quoteContext=menuProps.getString("QuotingContext");
	...
	$.ajax({
	      ...
	      success:  function(data){
						...
						if(response == 'firstTimeDistiSaved'){
										$.ajax({
										      type:  "POST",
										      url:   document.getElementById('crossTrackContext').value+"/requestStatusDispatch.do?methodName=saveDistiSigninAggrement",
										      async:  true,
										       data: "",
										      error:  function (request, msg, err){},
										      success:  function(data){ 
...
function openCriticalPopupwindow(msgid){	
	...
	window.open("bannerPage.do?methodName=bannerPopup&messageId="+id,"POPUP",dimen);
}
...
<input type="hidden" id="quoteContext" value="<%=quoteContext%>"/>
