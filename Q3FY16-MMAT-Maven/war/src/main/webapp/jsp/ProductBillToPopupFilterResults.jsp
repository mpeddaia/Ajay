<!-- JSPNAME = ProductBillToSelectPopup.jsp -->
<!--
								...
								if ((withinContext!=null) && (withinContext.equalsIgnoreCase("N"))){ 
									if((strBillToTabType!=null) && (strBillToTabType.equalsIgnoreCase("address"))) {  %>
										...
										<% if((strBillToTabType!=null) && (strBillToTabType.equalsIgnoreCase("search"))) {  %>
											...
											<td class="active">
												<a href="#" onclick="showSearchResultsNum()" align="absmiddle" id="tab1"><img src="images/global/tabs/ongrey/tab_left_first_selected.gif" alt="" align="absmiddle" border="0" /><span><fmt:message key='BillToSelector.searchResults'/></span><img src="images/global/tabs/ongrey/tab_right_selected.gif" alt="" align="absmiddle" border="0" /></a>
											</td>
										<% } else { %>
											<td>
												<a href="#" onclick="showSearchResultsNum()" align="absmiddle" id="tab1"><img src="images/global/tabs/ongrey/tab_left_first_selected.gif" alt="" align="absmiddle" border="0" /><span><fmt:message key='BillToSelector.searchResults'/></span><img src="images/global/tabs/ongrey/tab_right_selected.gif" alt="" align="absmiddle" border="0" /></a>
							...
							<c:when test="${billToResults != null}" >
								<% List billToResults1=(ArrayList)request.getAttribute("billToResults");
									if (billToResults1.size() > 0) { %>
											<th align="left" <%= sortTypeClassBillToID %> ><a href="javascript:showSearchPageData(1,'billtoId','<%= procId %>','<%=sortOrder%>','<%=prevSortId%>','pagination');"><fmt:message key='BillToSelector.Bill_To_Num'/></a></th>
											...
											<th align="left" <%= sortTypeClassParentGU %> ><a href="javascript:showSearchPageData(1, 'parentGu', '<%= procId %>','<%=sortOrder%>','<%=prevSortId%>','pagination');"><fmt:message key='BillToSelector.parentGu'/></a></th>
												...
												<% } %>
										<% } else { %>
											<th align="left" <%= sortTypeClassBillToID %> ><a ><fmt:message key='BillToSelector.Bill_To_Num'/></a></th>
							...
							<c:when test="${billToResults != null}" >
								<% List billToResults2=(ArrayList)request.getAttribute("billToResults");
									if (billToResults2.size() > 0) { %>
										<% if ((strAuthLevel != null) && (strAuthLevel.equalsIgnoreCase("4"))) {%>
											<th align="left" <%= sortTypeClassBillToID %> ><a href="javascript:showAddressBookData(1,'location','<%= procId %>','<%=sortOrder%>','<%=prevSortId%>','pagination');"><fmt:message key='BillToSelector.Bill_To_Num'/></a></th>
										<%} else { %>
											 <th align="left" <%= sortTypeClassBillToID %> ><a href="javascript:showAddressBookData(1,'billtoSiteId','<%= procId %>','<%=sortOrder%>','<%=prevSortId%>','pagination');"><fmt:message key='BillToSelector.Bill_To_Num'/></a></th>
										<% } %>
										<% if ((strAuthLevel != null) && (strAuthLevel.equalsIgnoreCase("4"))) {%>
