...
<%=erMsg%>
         <!-- grid header -->
            ...
            <tr valign="middle">
               ...
               <h2 style="color: #ffffff; font-size: 11px; font-weight: bold; line-height: 12px; white-space: nowrap; padding: 0; float: left;">Search Results</h2>  
               </td>
               <td align="right" valign="middle"  colspan=2>
               <c:choose>
               <c:when test="${(numPages==1) || (pageNo==1)}"> | &lt;&lt; | &lt; | <fmt:message key='SalesRegionSelector.pages'/> <c:out value="${numPages}"/></td><td> </c:when>
               <c:otherwise>
               | <a href="javascript:showSearchPageData(1, '<%= sortId %>','<%=procId%>','<%=revSortOrder%>','0');">&lt;&lt;</a> | <a href="javascript:showSearchPageData(<c:out value="${pageNo-1}"/>, '<%= sortId %>','<%=procId%>','<%=revSortOrder%>','0');">&lt;</a> | <fmt:message key='SalesRegionSelector.pages'/> <c:out value="${numPages}"/></td><td>
                </c:otherwise>
                 </c:choose>
               
                     <select name="searchPageNumber" id="searchPageNumber" size="1" class="inputTextSM" onChange="javascript:changeSearchPageData('<%= sortId %>','<%=procId%>');">
              ...
              <c:otherwise>
                  <tr class="watermelon">
                    ...
                    <% if (selectType.equals("1")){%>
                     <input type="checkbox" name="selectLineItem" id="selectLineItem" value="<c:out value="${entry.nodeName}"/>|<c:out value='${entry.structureNodeName}'/>" class="checkbox topGroup" onChange="refreshArray('<c:out value='${entry.nodeName}'/>|<c:out value='${entry.structureNodeName}'/>');"/>
                     <%} else {%>
                     <input type="radio" name="selectLineItem" value="<c:out value="${entry}"/>|<c:out value='${entry}'/>" onChange="refreshRadioArray('<c:out value='${entry.nodeName}'/>|<c:out value='${entry.structureNodeName}'/>');">
